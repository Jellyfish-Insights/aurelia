...
namespace ConsoleApp {
    ...
    class Program {
        ...
        static void Main(string[] args) {
            ...
            app.Command("transformer", (command) => {
                command.Description = "Execute selected transformer. All by default.";
                ...
                command.OnExecute(() => {
                    ...
                    var jobType = JobType.Transformation;
            ...
            app.Command("fetcher", (command) => {
                ...
                command.OnExecute(() => {
                    ...
                    var configuration = new Andromeda.Common.Jobs.JobConfiguration {
                        IgnoreAPI = ignoreApi.HasValue(),
                        MaxEntities = maxEntities.HasValue() ? Convert.ToInt32(maxEntities.Value()) : 0,
                        Paginate = !fetchAll.HasValue(),
                    };

                    var jobType = Andromeda.Common.Jobs.JobType.Fetcher;
                    var jobScope = Andromeda.Common.Jobs.JobScope.All;
                    if (selectedJobScope.HasValue()) {
                        if (!Enum.TryParse<Andromeda.Common.Jobs.JobScope>(selectedJobScope.Value(), out jobScope)) {
                            Console.WriteLine("Invalid job scope. Possible values are:");
                            foreach (var x in Enum.GetNames(typeof(Andromeda.Common.Jobs.JobScope))) {
                                Console.WriteLine($"\t{x}");
                            }
                            return 1;
                        }
                    ...
                    if (printDependencyTreeFlag.HasValue()) {
                        Andromeda.Commands.RunJobsCommand.PrintDependencyTree(jobType, jobScope, jobNames);
                    } else if (listAvailableJobsFlag.HasValue()) {
                        Andromeda.Commands.RunJobsCommand.ListAvailableJobs(jobType, jobScope, jobNames);
                    } else {
                        return Andromeda.Commands.RunJobsCommand.RunJobs(jobType, jobScope, jobNames, configuration, debugFlag.HasValue());
                    }
                    return 0;
                });
            });
            ...
            app.Command("migrate", (command) => {
                ...
                command.OnExecute(() => {
                    if (!lake.HasValue() && !analytics.HasValue() && !facebook.HasValue()) {
                        Console.WriteLine("No database selected");
                        return 1;
                    }
                    if (lake.HasValue()) {
                        Andromeda.Commands.MigrateCommand.MigrateDataLake();
                    }
                    if (facebook.HasValue()) {
                        Andromeda.Commands.MigrateCommand.MigrateFacebook();
                    }
            ...
            app.Command("zip-facebook-cache", (command) => {
                ...
                command.OnExecute(() => {
                    ...
                    Andromeda.Commands.ZipFacebooCacheCommand.ZipCache(zipDir.Value, zipName.Value);
                    return 0;
                });
            });
            ...
            app.Command("check-data-lake", (command) => {
                ...
                command.OnExecute(() => {
                    ...
                    if (tables.Any()) {
                        Andromeda.Commands.ContinuityReportCommand.Report(tables);
                    } else {
