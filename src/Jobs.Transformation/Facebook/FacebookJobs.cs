...
namespace Jobs.Transformation.Facebook {
    ...
    public abstract class FacebookJob : TransformationJob {
        ...
        public static IEnumerable<EntityUpdateParams<SourceAd>> ListAds(NpgsqlConnection conn, JobTrace trace, IQueryable<string> sourceVideosId) {
            using (var cmd = conn.CreateCommand())
            {
                cmd.CommandText = $@"
                    SELECT
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        var videoId = reader.OptPrim<long>("video_id").HasValue ? reader.OptPrim<long>("video_id").ToString() : null;
                        if (!sourceVideosId.Contains(videoId)) {
                            continue;
                        }
                        var log = new RowLog();
                        log.AddInput("ads",
